 ...
 */
{
    let init = function () {
        ...
        $(document).on('change', '#order_by', function (e) {
            ...
            if ($(this).attr('data-doc-type') === 'doc') {
                let param = {
                ...
                if ($(this).val() == 1) {
                    api = file_api + '?order_by_name=ok&order=asc'
                } else if ($(this).val() == 2) {
                    api = file_api + '?order_by_name=ok&order=desc'
                } else if ($(this).val() == 3) {
                    api = file_api + '?order_by_date=ok&order=asc'
                } else if ($(this).val() == 4) {
                    api = file_api + '?order_by_date=ok&order=desc'
                }
                $.ajax({
                    method: 'GET',
                    url: api,
                ...
                $pagination.hide();
                let api = category_api + '?id=' + doc_id;

                if ($(this).val() == 1) {
                    api = category_api + '?id=' + doc_id + '&order_by_name=ok&order=asc'
                } else if ($(this).val() == 2) {
                    api = category_api + '?id=' + doc_id + '&order_by_name=ok&order=desc'
                } else if ($(this).val() == 3) {
                    api = category_api + '?id=' + doc_id + '&order_by_date=ok&order=asc'
                } else if ($(this).val() == 4) {
                    api = category_api + '?id=' + doc_id + '&order_by_date=ok&order=desc'
                }
                $.ajax({
                    method: 'GET',
                    url: api,
        ...
        $('#left').on('changed.jstree', function (e, data) {
            ...
            if (selectedItem.length) {
                $(selectedItem).each(function (k) {
                    ...
                    $('#order_by').attr('data-doc-type', type);
                    $('#order_by').attr('data-docid', id);
                    if (type == 'cat' || type == 'root') {
                        //hide pagination
                        var selectobject;
                        selectobject = document.getElementById("order_by").getElementsByTagName("option");
