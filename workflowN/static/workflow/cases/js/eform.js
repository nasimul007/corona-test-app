...
function CommonFormSet(task_id, app_id, query_id, delegation_id, auto) {
    var ab_idd = app_id;
    ...
    var $new_form, $hide_form;
    if (auto === true) {
        $new_form = $('#bulk_submit_form');
        $hide_form = $('#new_form');
    } else {
        $new_form = $('#new_form');
        $hide_form = $('#bulk_submit_form');
    }
    ...
    function UpdateStatus(current, status, comment,) {
        console.log("in", current);
        var appp = $new_form.find('input[name=application]').val();
        console.log(appp);
        var item_row = $("#bulk_submit_table tbody").find("tr[data-app_id='" + current + "']");
        item_row.find('td.status').html(status)
        item_row.find('td.comment').html(comment);
        inboxdt.clearPipeline().draw();
    }
    ...
    $(document).off('click.rew').on('click.rew', submit_button_group, function (e) {
        ...
        if (clickbutton === 'save_as_draft') {
            ...
            if (process_form.isValid()) {
                ...
                if (auto === true) {
                    var current_app = $new_form.find('input[name=application]').val();
                    UpdateStatus(current_app, '<i class="zmdi zmdi-close-circle c-red f-20"></i>', 'You have some required fields');
                    // var app = $new_form.find('input[name=application]').val()
                    // var item_row = $("#bulk_submit_table tbody").find("tr[data-app_id='" + app +"']");
                    // item_row.find('td.status').text('Failed')
                    // item_row.find('td.comment').text('You have some required fields');
    ...
    function FormSetUP(data) {
        if (data.type === 'eform' || data.type === 'query_view' || data.type === 'history_view') {
            ...
            //readonly selectpicker
            $new_form.find('select[class=selectpicker][readonly]').each(function () {
                $(this).closest('.btn-group').find('.dropdown-toggle').attr('disabled', true)
            });

            // readonly and disable grid fields
            function GridReadonly() {
                let $grids = $("div[class=grid]");
                if ($grids.length) {
                    $grids.find('input, select').each(function (j, element) {
                        //except selectpicker
                        if ($(element).is('input') && !$(element).parent().hasClass('bs-searchbox') && $(element).attr('readonly')) {
                            let $current = $(element).val();
                            $(element).closest('.form-group').hide();
                            $(element).closest('td').append($current);
                        } else if ($(element).is('select') && $(element).attr('readonly')) {
                    ...
                    $grids.each(function (i, ele) {
                        if ($(ele).attr('data-view-mode') === 'view') {
                            $(ele).find('.add_div').remove();
                            $(ele).find('tr').find('td:last, th:last').hide();
                        }
                    });
            ...
            if (extra_ == 'draft') {
                $new_form.find('input[name=_approve_]').attr('value', 'Submit');
            ...
            if (data.init_task === true) {
                $new_form.find('input[name=_recheck_]').closest('.form-group').parent().remove();
            }
            ...
            //auto submit
            if (auto === true) {
                $new_form.find("input[name='_approve_']").click();
            }

            //script
             eval(data.eform.script)
            ...
            if (data.end == true) {
                ...
                if (auto === true) {
                    var current_app = $new_form.find('input[name=application]').val();
                    var comment_detail = data.detail;
                    (data.upload_dms == true) ? comment_detail = comment_detail + "<br> <a class='btn btn-xs btn-info' " +
                        "href='/dms/document/upload/standard_upload/filesave/application/" + data.application.id + "'" +
                        "target='_blank'>upload to DMS</a>" : '';
                    UpdateStatus(data.application.id, '<i class="zmdi zmdi-check-circle f-20 c-green"></i>', comment_detail);
                } else {
                    swal({
                        title: "Good Job",
                        text: data.detail,
                ...
                if (data.manual == true) {
                    if (auto === true) {
                        UpdateStatus(data.application.id, '<i class="zmdi zmdi-close-circle c-red f-20"></i>', 'This is a manual assignment. Please submit manually');
                    } else {
                    ...
                    if (data.parallel == true) {
                        if (auto === true) {
                            var current_app = $new_form.find('input[name=application]').val();
                            UpdateStatus(data.application.id,
                                '<i class="zmdi zmdi-check-circle f-20 c-green"></i>',
                                "Multiple users have been assigned to the next task as it is going as parallel.")
                        } else {
                            swal({
                                title: "Good Job",
                        ...
                        if (data.recheck == true || data.reject == true) {
                            ...
                            if (auto === true) {
                                var current_app = $new_form.find('input[name=application]').val();
                                UpdateStatus(data.application.id, '<i class="zmdi zmdi-check-circle f-20 c-green"></i>',
                                    "<b>Next Task :</b> " + data.target + ", <b>Next User :</b> " + data.user)
                            } else {
                                swal({
                                    title: "Good Job",
        ...
        $.ajax({
            ...
            error: (res) => {
                if (auto === true) {
                    var error_comment = '';
                    $.each(JSON.parse(res.responseText), (k, v) => {
                        error_comment = error_comment + ' ' + v
                    });
                    // console.log("0000", $formData.get('application'));
                    UpdateStatus($formData.get('application'), '<i class="zmdi zmdi-close-circle c-red f-20"></i>',
                        error_comment)
                } else {
                    $.each(JSON.parse(res.responseText), (k, v) => {
                        notify('Sorry!! ', v, '', 'danger', 10000);
