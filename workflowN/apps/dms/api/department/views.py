...
def getBranch(request):
    root_nodes = cache_tree_children(Branch.objects.all())
    dicts = []
    for n in root_nodes:
        dicts.append(recursive_node_to_dict(n, 'branch'))
    return Response(dicts, status=200)
...
def recursive_node_to_dict(node, id=None):
    if id is not None:
        result = {
            'id': node.id,
            'text': node.name,
    ...
    if atype == 'manager':
        ...
        if action == 'add':
            if userid:
                ...
                try:
                    if indicator == 'dept':
                        ...
                        departmentdata = Branch.objects.get(id=dept)
                        userdata = User.objects.filter(id__in=userid).update(branch=departmentdata)

                        userqueryset = User.objects.filter(branch=departmentdata)
                    serializer = UserInfoSerializer(userqueryset, many=True)
                    # Audit Trail
                    users = []
                    for uid in userid:
                    ...
                    indicator_str = ""
                    if indicator == 'dept':
                        indicator_str = 'Department'
                    else:
                        indicator_str = 'Branch'
                    description = "User: " + users_str + " has been assigned to" + indicator_str+ ": "+ dept
                    x_forwarded_for = request.META.get('HTTP_X_FORWARDED_FOR')
                    ...
                    if x_forwarded_for:
                        ...
                        ip_address = request.META.get('REMOTE_ADDR')
                    DmsActivity(user=current_user, operation=indicator_str +" Assign User",
                                ip=ip_address,
                                description=description, activity_time=timezone.now()).save()
            ...
            if indicator == 'dept':
                ...
                User.objects.filter(id__in=userid).update(branch=None)
                departmentdata = Branch.objects.get(id=dept)
                userqueryset = User.objects.filter(branch_id=dept)
            # if user is manager
            try:
                indicator_str = 'Branch'
                if indicator == 'dept':
                ...
                users_str = ", ".join(users)
                dept = str(departmentdata)
                description = "User: " + users_str + " has been removed from "+indicator_str+": " + dept
                x_forwarded_for = request.META.get('HTTP_X_FORWARDED_FOR')
                ...
                if x_forwarded_for:
                    ...
                    ip_address = request.META.get('REMOTE_ADDR')
                DmsActivity(user=current_user, operation=indicator_str+" Remove User",
                            ip=ip_address,
                            description=description, activity_time=timezone.now()).save()
...
class getFreeStaffBranchViewSet(viewsets.ModelViewSet):
    # permission_classes = [permissions.DjangoModelPermissions]
    pagination_class = LargeResultsSetPagination
    serializer_class = UserInfoSerializer
    http_method_names = ['get']
    queryset = User.objects.filter(branch=None)
...
class BranchViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        description = ""
        if parent is None:
            description = "Branch: '" + serializer.data['name'] + "' has been Created"
        else:
            parent_name = Department.objects.get(id=parent).name
            description = "Branch: '" + serializer.data['name'] + "' has been Created under: " + parent_name
        x_forwarded_for = self.request.META.get('HTTP_X_FORWARDED_FOR')
        ...
        if x_forwarded_for:
            ...
            ip_address = self.request.META.get('REMOTE_ADDR')
        DmsActivity(user=self.request.user, operation="Create Branch", ip=ip_address,
                    description=description, activity_time=timezone.now()).save()
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        description = "Branch: '" + str(instance) + \
                      "' has been Deleted"
        x_forwarded_for = self.request.META.get('HTTP_X_FORWARDED_FOR')
        ...
        if x_forwarded_for:
            ...
            ip_address = self.request.META.get('REMOTE_ADDR')
        DmsActivity(user=self.request.user, operation="Branch Delete", ip=ip_address,
                    description=description, activity_time=timezone.now()).save()
        return Response(status=status.HTTP_204_NO_CONTENT)
