...
import io
from conf.settings import CLIENT_NAME
    ...
    try:
        ...
        if isinstance(result, Documents):
            ...
            for tempmata in tempmetas:
                ...
                if CLIENT_NAME == 'NRBC':
                    if tempmata['name'] == 'date' or tempmata['name'] == 'voucher_date':
                        if tempmata['value'] == "":
                            d.pop(tempmata['name'])
            # process
...
class DocumentListViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if docid is not None:
            ...
            queryset = []
        if queryset:
            if order_by_name is not None and order == 'asc':
                queryset = queryset.order_by('filename')
            elif order_by_name is not None and order == 'desc':
                queryset = queryset.order_by('-filename')
            elif order_by_date is not None and order == 'asc':
...
def DocumentSummaryViewSet(request):
    ...
    if current_user.role_id == 1:
        returndata = Documents.objects.filter(deleted=False, published=True).values('doc_type').annotate(
            name=Concat('doc_type__name', Value('')), y=Count('doc_type'))
...
class UploadReportViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        date_from = (self.request.query_params.get('columns[1][search][value]', None))
        date_to = (self.request.query_params.get('columns[2][search][value]', None))
        doc_type_id = self.request.query_params.get('doc_type_id', None)
        option = self.request.query_params.get('option', None)
        x_forwarded_for = self.request.META.get('HTTP_X_FORWARDED_HOST')
        host = self.request.META.get('HTTP_HOST')
        name = self.request.user.get_full_name()
        to = self.request.user.email
        date_from_url = self.request.query_params.get('date_from')
        date_to_url = self.request.query_params.get('date_to')
        print(date_from_url)
        # dateRange search
        if date_from and date_from is not None:
            if date_to and date_to is not None:
        ...
        if doc_type_id:
            ...
            if date_from_url and date_to_url:
                date_from_url = date_from_url.replace(" ", "+")
                date_to_url = date_to_url.replace(" ", "+")
                queryset = queryset.filter(uploaded_at__range=[date_from_url, date_to_url])
        if option == 'download' and doc_type_id is not None:
            DMSTasks.upload_report_export.delay(doc_type_id, self.request.user.id, x_forwarded_for, host, name, to, date_from_url, date_to_url)
