...
class RootSerializer(serializers.ModelSerializer):
    ...
    docs_count = serializers.SerializerMethodField('get_doc_count')
    ...
    def get_doc_count(self, obj):
        print('id', obj.id)
        docs = Documents.objects.filter(doc_type_id=obj.id, deleted=False, archived=False, locked=False,
                                        parent__isnull=True)
        return docs.count()
...
class SiblingsSerializer(serializers.ModelSerializer):
    ...
    def get_sibling(self, obj):
        ...
        return [{
            ...
            'docs_count': Documents.objects.filter(doc_type_id=sibling.id, deleted=False, archived=False, locked=False,
...
class CategorySerializer(serializers.ModelSerializer):
    ...
    def get_childrens(self, obj):
        return [{
            ...
            'docs_count': Documents.objects.filter(doc_type_id=child.id, deleted=False, archived=False, locked=False,
                                        parent__isnull=True).count(),
    ...
    def update(self, request, *args, **kwargs):
        ...
        if retention_type:
            if retention_type == '0':
                ...
                category_instance.expiry_date = datetime.date.today() + datetime.timedelta(
                    int(retention_month) * 365 / 12)
                category_instance.save()
        x_forwarded_for = request.META.get('HTTP_X_FORWARDED_FOR')
