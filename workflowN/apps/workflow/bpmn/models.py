    ...
    def save(self, *args, **kwargs):
        ...
        if self.content is not None:
            ...
            for row in content['items'][0]['items']:
                for columns in row:
                    # if variable is not exit in Variable table, creat variable to Variable table
                    if 'variable' in columns and columns['variable'] is not None:
                        try:
                            Variable.objects.get(project=self.project, name=columns['name'])
                        except Variable.DoesNotExist:
                            Variable.objects.create(name=columns['name'], type=columns['dataType'], project=self.project)
...
class Application(models.Model):
    ...
    initiated_branch = models.ForeignKey(Branch, blank=True, null=True)
...
class EmailQueues(models.Model):
    from_e = models.EmailField(null=False, blank=False, default=settings.DEFAULT_FROM_EMAIL)
...
class FailedEmailQueues(models.Model):
    from_e = models.EmailField(null=False, blank=False, default=settings.EMAIL_HOST_USER)
    to = models.EmailField(null=False, blank=False)
    cc = models.TextField(null=True, blank=True)
    subject = models.TextField(null=False, blank=False)
    body = models.TextField(null=False, blank=False)
