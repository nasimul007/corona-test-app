...
class BPMNEngine(object):
    ...
    def create_application(self):
        if self.out_open == 'True':
            ...
            bool_out_open = False
        branch = None;
        if self.request.user.branch is not None:
                ...
                for i in range(position, max_position + 1):
                    try:
                        ...
                        if single_step.type == 0 and single_step.position == position:
                            ...
                            try:
                                Route.objects.get(project_id=self.instance.project.id,
                                                  target_id=self.instance.id,
            ...
            if self.position == -1 or max_position is None or max_position == 0:
                ...
                if self.next_assignment():
                    if self.finish_application:
                        self.application.finished_at = datetime.now()
                        ...
                        if self.button_action == 'recheck':
                            ...
                            self.current_delegation.additional_status = 0
    ...
    def workflow_send_mail(self, user=None, button_action=None, application_end=None, delegation_status=None):
        application_data = json.loads(self.application.data)
        # get the label of dropdown and radio
        for k, v in application_data.items():
            try:
                variable_info = Variable.objects.get(project_id=self.application.project.id, name=k)

                if variable_info.accepted_defaults:
                    options = variable_info.accepted_defaults.replace("\'", "\"")
                    json_options = json.loads(options)
                    for single in json_options:
                        if single['value'] == v:
                            application_data[k] = single['label']

            except Exception as ex:
                logger = logging.getLogger('warning_logger')
                logger.error('{}'.format(ex.__str__()))

        # delegatee assignment mail
        if user is not None:
            ...
            html_content = render_to_string('email/delegatee_email.html',
                                            {
                                                'email_body': '{0} would like to kindly inform you that '
                                                              'your task is being assigned to your delegatee'.format(
        ...
        # task rejection mail
            ...
            html_content = render_to_string('email/delegatee_email.html',
                                            {
                                                'email_body': '{0} would like to kindly inform you that '
                                                              'your initiated task has been rejected by'.format(licensed.PRODUCT_NAME),
        ...
        #     task complete mail
            ...
            if self.application.out_open is True:
                ano_mail_content = render_to_string('email/application_completed_email.html',
                                                    {
                                                        'company_name': licensed.CLIENT_COMPANY_NAME,
                ...
                EmailQueues.objects.create(**{
                    ...
                    'to': json.loads(self.application.data).get('email'),
                    'cc': '',
                    'subject': 'Account Opening',
                    'body': html.escape(ano_mail_content)
                })
        # non-claim email
        elif delegation_status is not None and delegation_status == 3:
            subject, from_email, to = 'You have a task in your queue', self.request.user.email, self.next_user.email
            non_claim_link = '{}/workflow/case/non_claimed'.format(self.request.META['HTTP_HOST'])
            html_content = render_to_string('email/next_workflow.html',
                                            {
                                                'next_user': self.next_user.get_full_name(),
                                                'sent_by': self.request.user.get_full_name(),
                                                'non_claim_link': non_claim_link,
                                                'init_date': self.application.created_at.strftime('%d %B, %Y'),
                                                'due_date': BPMNEngine.due_date(self.next_task).strftime('%d %B, %Y'),
                                                'project_name': self.next_task.project.title,
                                                'app_number': self.application.number,
    ...
    def next_task_assignment_op(self, task, routing_type, button_action=None, user=None):
        ...
        for index in range(len(users)):
            ...
            d, created = Delegation.objects.get_or_create(defaults=defaults, **lookups)
            self.next_task = task
            if is_delegate_available.exists():  # check if delegatee assigned # azmi
            ...
            if self.button_action != 'recheck':
                self.add_backtrack_path(self.next_user, self.next_task)
            self.workflow_send_mail(None, None, None, d.status)

            if is_delegate_available.exists():  # check if delegatee assigned # azmi
                self.workflow_send_mail(user, None, None, d.status)
