...
class EForm(object):
    ...
    def generate_form(self, preview=False, col_mode='md'):
        ...
        for r in row:
            ...
            # create column
            for el in r:
                # print("row", el.get('type'), el.get('name'))
    ...
    def label(obj):
        ...
        return "<label class='{2} {4}' id='{3}-label'> {0}</label>".format(label_text, required_class, label_class, obj.get('id'), margin)
    ...
    def html_property(obj):
        ...
        # added to solve radio button and dropdown view only mode bug solve
        if obj.get('type') == 'radio':
            if field_mode == 'disabled' or field_mode == 'readonly':
                field_mode = "onclick='javascript: return false;'"
        if obj.get('type') == 'dropdown':
            if field_mode == 'disabled' or field_mode == 'readonly':
                field_mode = "disabled='disabled'"

        # remove id attr for radio type
        element_id = "id='{0}'".format(obj.get('id'))
        if obj.get('type') == 'radio':
            element_id = ''
        return '''
            name='{0}' placeholder='{1}' {2} {3} {4} {5} {6} {7}
        '''.format(name, obj.get('placeholder'), max_length, EForm.required(obj),
                   value, element_id, field_type, field_mode)
    ...
    def text_field(obj):
        ...
        if obj.get('type') == 'datetime':
            ...
            datetime = ''
            return '''
                    {0}<input class='form-control {2}' {1} data-formula="{3}">
                    '''.format(EForm.label(obj), EForm.html_property(obj), datetime, obj.get('formula')).replace('\n', '')
    ...
    def file_field(obj):
        if obj.get('mode') == 'view':
            ...
            return '''
            <input type='file' id='{2}' style="display:none" disabled='disabled'>
            <p class='f-500  m-0'>{0}</p>
            {1}
            '''.format(obj.get('label'), anchor, obj.get('id'))
        ...
        return '''
        <label class="fg-label" style="display:table-cell">{0}</label>
        ...
        if len(obj.get('options')) > 0:
            ...
            for e in obj.get('options'):
                if e.get('value') == "1":
                    ...
                    select += "<div class='checkbox m-b-5'>" \
    ...
    @staticmethod
    def title_field(obj):
        return "<h4 {1} class='text-center m-t-5 m-b-5 c-black'>{0}</h4>".format(obj.get('label'), EForm.html_property(obj))
...
class GridItem:
    ...
    def final_table(self):
        return '''
        <div class='grid' data-view-mode='{mode}'>
    ...
    def get_mode(self):
        if self.field['mode'] == 'disabled':
            field_mode = "disabled"
        elif self.field['mode'] == 'view':
            field_mode = "readonly"
        elif self.field['mode'] == 'parent':
            if self.grid_object['mode'] == 'disabled':
                field_mode = "disabled"
            elif self.grid_object['mode'] == 'view':
                field_mode = "readonly"
            else:
                field_mode = ""
        else:
    ...
    def common_attributes(self):
        ...
        return "type='{type}' name={name} class='{class}' {viewMode} placeholder='{placeholder}' " \
               "maxLength='{maxLength}' {required} value='{value}'".format(**attributes)
    ...
    def text_area_field(self):
        ...
        return "<textarea class='form-control auto-size' {} rows='{}' name='{}' {}>{}</textarea>".format(
            self.get_mode(), self.field['rows'], self.get_name(), self.set_required(), value_or_placeholder)
    ...
    def dropdown_field(self):
        ...
        return '''<select class='selectpicker' {viewMode} data-live-search="true" value="{selected-value}"
        name={name} {required}>
