        ...
        for k, v in self.application.items():
            if k == 'data':
                for i in json.loads(v):
                    if not isinstance(json.loads(v)[i], (list,)) and json.loads(v)[i] != "":
                        doc_type.add_data({i: json.loads(v)[i]})
                #data_dic = json.loads(v)
                # remove element from elastic
                # if 'name' in data_dic:
                #     del data_dic['name']

                #doc_type.add_data({i: data_dic[i] for i in data_dic if 'file' not in i})
