...
from apps.core.api.Pagination import LargeResultsSetPagination, CustomLargeResultsSetPagination
...
class TaskViewSet(CustomViewSetForQuerySet):
    ...
    def retrieve(self, request, *args, **kwargs):
        ...
        instance = BPMNEngine(request=request, instance=instance, position=position, start_app=start_app,
                              application=application, out_open=out_open).take_decision()
...
class EFormViewSet(CustomViewSetForQuerySet):
    ...
    def list(self, request, *args, **kwargs):
        ...
        is_importing = self.request.query_params.get('import', None)

        if project is None:
            raise serializers.ValidationError({'detail': 'Please set project parameter'})
        if is_importing is None:
    ...
    def update(self, request, *args, **kwargs):
        ...
        data_set = json.loads(serializer.validated_data["content"])
        for row in data_set["items"][0]["items"]:
            for element in row:
                if 'name' in element and 'dataType' in element:
                    element_update = Variable.objects.update_or_create(
                        project_id=serializer.validated_data['project'].id,
                        name=element["name"],
                    )[0]
                    element_update.type = element["dataType"]
                    if 'options' in element:
                        element_update.accepted_defaults = element["options"]
                    try:
                        element_update.save()
...
class ApplicationViewSet(CustomViewSetForQuerySet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            ...
            logger.warning('{}'.format(e.__str__()))
            return Response('There is something wrong with indexing system. Please contact with support team.',
        ...
        if request.data.get('save_as_draft') and int(request.data.get('save_as_draft')) == 1:
            ...
            # print("condition", task.project.title, task.name)
            if licensed.CLIENT_NAME == 'aw6io2a' and 'EBL CARD CUSTOMER SERVICE' in task.project.title and 'Documentation' not in task.name:
...
class ProjectNotificationView(DetailView):
    template_name = 'workflow/bpmn/project_notification.html'
    model = Project
...
class CountViewSet(CustomViewSetForQuerySet):
    ...
    def list(self, request, *args, **kwargs):
        ...
        if total != 0:
            ...
            recheck = Delegation.objects.filter(
                Q(user=self.request.user) & Q(status=0) & Q(additional_status=1)).count()
        ...
        initiated = Application.objects.filter(init_user=self.request.user,
        ...
        return Response({
            ...
            'total': total,
            'query': query_count,
            'risk': Delegation.objects.filter(user=self.request.user, status=1, risk_date__lte=timezone.now(),
                                              due_date__gt=timezone.now()).count(),
...
def workflow_send_mail(identifier_query, appquery):
    application_data = json.loads(appquery.application.data)
    print("query data", application_data)
    if identifier_query == 'question':
        ...
        subject_body = '{0} would like to kindly inform you that,' \
                       ' you have a new query regarding a task that you have already completed from'.format(
            licensed.PRODUCT_NAME)
    else:
        is_answer = True
        ...
        subject_body = '{0} would like to kindly inform you that,' \
                       'answer for your query has been submitted by'.format(licensed.PRODUCT_NAME)
    ...
    try:
        ...
        EmailQueues.objects.create(**{
            'from_e': licensed.EMAIL_HOST_USER,
            'to': to,
            'cc': '',
            'subject': subject,
            'body': html.escape(html_content)
        })
    except Exception as e:
        logger = logging.getLogger('warning_logger')
...
class AppMonitorSerializer(serializers.ModelSerializer):
    init_user = serializers.StringRelatedField(source='init_user.get_full_name')
    current_user = serializers.StringRelatedField(source='current_user.get_full_name')
    project = serializers.StringRelatedField(source='project.title')
    comment_count = serializers.SerializerMethodField()

    @staticmethod
    def get_comment_count(obj):
        return AppComment.objects.filter(application=obj.id).count()
...
class AppMonitorViewSet(CustomViewSetForQuerySet):
    ...
    pagination_class = LargeResultsSetPagination
    queryset = Application.objects.filter(finished_at__isnull=True)
    model = Application
    permission_id = [3, 25, 26]
    search_keywords = ['number', 'project__title', 'current_user__first_name', 'current_user__last_name',
                       'init_user__first_name', 'init_user__last_name']
        ...
        if all is not None and self.request.user.role.id == 1:
            ...
            for p in self.request.user.role.permission.all():
                if p.id == 26:
                    ...
                    break;
            if has_permission == 0:
                raise serializers.ValidationError({'You do not permission to perform this action'})
        else:
            ...
            queryset = self.queryset.filter(project__id__in=projects_array, finished_at__isnull=True).order_by('-id')
    ...
    def create(self, request, *args, **kwargs):
        ...
        if is_submit is None or not is_submit:
            ...
            if expiry_date is not None:
                ...
                app_query = self.model(query=query, user_from=User.objects.get(pk=user_from),
                                       ...
                                       task=Task.objects.get(pk=task), expiry_date=expiry_date_date)
            else:
                app_query = self.model(query=query, user_from=User.objects.get(pk=user_from),
                                       user_to=User.objects.get(pk=user_id),
                                       delegation=Delegation.objects.get(pk=delegation_id),
...
class DelegationReportViewset(CustomViewSetForQuerySet):
    permission_classes = [GreenOfficeApiBasePermission]
    serializer_class = DelegationSerializer
    pagination_class = CustomLargeResultsSetPagination
    queryset = Delegation.objects.all()
    http_method_names = ['get']
    model = Delegation
